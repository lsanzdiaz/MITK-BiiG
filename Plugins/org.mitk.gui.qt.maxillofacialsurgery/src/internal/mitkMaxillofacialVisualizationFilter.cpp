...
mitk::MaxillofacialVisualizationFilter::MaxillofacialVisualizationFilter()
...
void mitk::MaxillofacialVisualizationFilter::SetAssociatedSurfaceModel(unsigned int idx, BaseData* data)
{
...
*/
const mitk::BaseData* mitk::MaxillofacialVisualizationFilter::GetAssociatedSurfaceModel(unsigned int idx)
{
...
void mitk::MaxillofacialVisualizationFilter::SetRotationMode(RotationMode r)
{
	m_RotationMode = r;
}
...
*/
void mitk::MaxillofacialVisualizationFilter::GenerateData()
{
	...
	for (unsigned int index = 0; index < this->GetNumberOfIndexedInputs(); index++)
	{
		...
		/*****************************Modify geometry associated surface model data ********************************/
		
		const mitk::BaseData* surface_model_data = this->GetAssociatedSurfaceModel(index);

		if (surface_model_data == NULL)
		{
			MITK_WARN << "No BaseData associated with input " << index;
		}
		...
		else
		{
			...
			//store the current scaling to set it after transformation
			mitk::Vector3D surface_model_spacing = surface_model_data->GetGeometry()->GetSpacing();
			//clear spacing of data to be able to set it again afterwards

			surface_model_data->GetGeometry()->SetSpacing(scale);

			//set the original spacing to keep scaling of the surface model geometry

			if (offset.IsNotNull()) //first use offset if there is one.
			{
				mitk::AffineTransform3D::Pointer overallSurfaceTransform = mitk::AffineTransform3D::New();
				overallSurfaceTransform->SetIdentity();
				if (GetSurfaceTransform(index).IsNotNull())
				{
					overallSurfaceTransform->Compose(GetSurfaceTransform(index));
				}
				overallSurfaceTransform->Compose(affineTransform);
				overallSurfaceTransform->Compose(offset);
				surface_model_data->GetGeometry()->SetIndexToWorldTransform(overallSurfaceTransform);
			}
			else
			{
				mitk::AffineTransform3D::Pointer overallSurfaceTransform = mitk::AffineTransform3D::New();
			...
			surface_model_data->GetGeometry()->SetSpacing(surface_model_spacing);
			surface_model_data->GetGeometry()->TransferItkToVtkTransform(); // update VTK Transform for rendering too
			surface_model_data->GetGeometry()->Modified();
			surface_model_data->Modified();
		}
		/****************************************************************************************************/
...
* \brief Transfer the information from the input to the associated BaseData. This method changes with respect to the method of the base class.
*/
void mitk::MaxillofacialVisualizationFilter::SetSurfaceTransform(int index, mitk::AffineTransform3D::Pointer surface_transform)
{
	m_SurfaceTransformList[index] = surface_transform;
}

mitk::AffineTransform3D::Pointer mitk::MaxillofacialVisualizationFilter::GetSurfaceTransform(int index)
{
