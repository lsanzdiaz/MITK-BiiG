...
void mitk::NavigationDataRecorder::Update(double min_dist)
{
	if (m_Recording)
	{
		...
		if (m_OutputFormat == mitk::NavigationDataRecorder::csv)
		{
			...
			if (m_firstLine)
			{
				...
				*m_Stream << "TimeStamp";
				...
				{
					*m_Stream << ";Valid_Tool" << index << ";X_Tool" << index << ";Y_Tool" << index << ";Z_Tool" << index << ";QX_Tool" << index <<
						";QY_Tool" << index << ";QZ_Tool" << index << ";QR_Tool" << index << ";Min_Dist" << index;
				}
				*m_Stream << "\n";
			}
		...
		for (unsigned int index = 0; index < inputs.size(); index++)
		{
			...
			if (timestamp >= 0)
			{
				if (this->m_OutputFormat == mitk::NavigationDataRecorder::xml)
				{
					...
					elem->SetDoubleAttribute("Min_Dist", min_dist);
				...
				{
					*m_Stream << ";" << dataValid << ";" << position[0] << ";" << position[1] << ";" << position[2] << ";" << orientation[0] << ";" << orientation[1] << ";" << orientation[2] << ";" << orientation[3] << ";" << min_dist;
				}
			}
		}
