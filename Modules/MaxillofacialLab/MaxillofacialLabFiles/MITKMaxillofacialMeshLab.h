...
#ifndef MITKMaxillofacialMeshLab_h
#define MITKMaxillofacialMeshLab_h
...
namespace mitk
{
  namespace ACVD
  {
     ...
     */
    MitkMaxillofacialLab_EXPORT Surface::Pointer Remesh(Surface::ConstPointer surface, unsigned int t, int numVertices, double gradation, int subsampling = 10, double edgeSplitting = 0.0, int optimizationLevel = 1, bool forceManifold = false, bool boundaryFixing = false);

    /** \brief Encapsulates mitk::ACVD::Remesh function as filter.
     */
    class MitkMaxillofacialLab_EXPORT RemeshFilter : public mitk::SurfaceToSurfaceFilter
    {
    public:
	  ...
	  void GetControlDistanceMatrix();
	  vtkSmartPointer<vtkSurface> GetMeshSurface();
	  mitk::Surface::Pointer* GenerateMeshWithMargin(mitk::Surface::ConstPointer surface, unsigned int t, int numVertices, double gradation, int subsampling, double edgeSplitting, int optimizationLevel, bool forceManifold, bool boundaryFixing);
	  mitk::Surface::Pointer GenerateMesh(mitk::Surface::ConstPointer surface, unsigned int t, int numVertices, double gradation, int subsampling, double edgeSplitting, int optimizationLevel, bool forceManifold, bool boundaryFixing);

	  //double* CreateControlDistanceMatrix(mitk::Point3D P, vtkSmartPointer<vtkSurface> surface);
	  unsigned int CreateControlDistanceMatrix(mitk::Point3D P, vtkSmartPointer<vtkSurface> surface);
	  void InsertVtkPolydata(vtkSmartPointer<vtkSurface> remesherOutput, vtkSmartPointer<vtkPolyData> MarginSurfacePolyData);
	  
	  void SetToolPositionMatrix(mitk::Point3D pr, double n);
	  void SetSurfacePointsMatrix(vtkSmartPointer<vtkSurface> surface);
	  MatrixControl *GetDifferenceMatrix();
	  double *mitk::ACVD::RemeshFilter::MonitorDistance(mitk::Point3D P);
	  MatrixControl *GetToolPositionMatrix();
    protected:
      void GenerateData();
    ...
    private:
	  ...
	  vtkSmartPointer<vtkSurface> m_Surface;
	  MatrixControl *m_PositionMatrix;
	  MatrixControl *m_SurfaceMatrix;
	  MatrixControl *m_DifferenceMatrix;
